...
function destinatonItemFunc(option='', id='', type=1, latitude='', longitude='', price=0) {
    ...
    item =
        ...
        '                     <input type="text" name="address[]" required value="'+option+'" class="form-control address-search input-roundless preventEnter" autocomplete="off" onclick="this.select();">\n' +
...
function reinitTaxi() {
                    ...
                    success: function (data) {
                        ...
                        if (len > 0) {
                            for (var i = 0; i < len; i++) {
                                ...
                                box_search.parent().find("#search_result_taxi").append('<li data-id="' + data['taxies'][i]["id"] + '" data-taxi-status="' + taxiStatus + '" style="background-color: '+rowColor+'55">' + data['taxies'][i]["code"] + ' - ' + data['taxies'][i]['firstname'] + ' ' + data['taxies'][i]['lastname'] + '</li>').fadeIn();
                            }
                        }
                    }
                })
    ...
    $('body').delegate('#search_result_taxi li', "click", function() {
        taxiStatus = $(this).data('taxiStatus');
...
if(requestMethod=='post'){
    $(function(){
        ...
        $( ".address-search" ).each(function(index) {
            ...
            destinationID = $(this).next().val();
            destinationType = $(this).next().next().val();
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data['success']) {
                            ...
                            for (var i = 0; i < len; i++) {
                                selectedStr = '';
                                if (street == data['numbers'][i]['number']) selectedStr = 'selected';
                                addressBox.next().find('select[name="number_street[]"]').append('<option ' + selectedStr + ' data-longitude="' + data['numbers'][i]["longitude"] + '" data-latitude="' + data['numbers'][i]["latitude"] + '">' + data['numbers'][i]['number'] + '</option>').fadeIn();
                            }

                            $('.select-search').select2();

                            addressBox.next().find('select:first-child').trigger('change');
                        }
                    }
                })
            }else{
...
$('body').delegate('#customer_phone', "keyup", function() {
    ...
    if(customerPhone.length==12){
        $.ajax({
            url: baseUrl+'/operator/order/ajaxGetLastOrders',
            dataType: 'json',
            data: {
            ...
            success: function (data) {
                ...
                if(data['success']){
                    ...
                    $('#customer__group_name').text(data['customer_group_name']);
                }
                else
                {
                    $('#customer_id').val("");
                    $('#customer_name').val("");
                    $('#customer__group_name').text("Ümumi");
                }
            }
        });
    }else{
...
$('body').delegate('.executeHistory', "click", function(){
    ...
    if($('select[name="order_type"] option:checked').val()=="2") {
        var text = "";
        if($($routes[0]).data('street')) text+= '('+$($routes[0]).data('street')+') ';
        text+= $($routes[0]).text()
        destinatonItemFunc(text, $($routes[0]).data('id'), $($routes[0]).data('type'), $($routes[0]).data('lat'), $($routes[0]).data('lng'), $($routes[0]).data('price'));
        ...
        for (i = 0; i < $routes.length; i++) {
            var text = "";
            if($($routes[i]).data('street')) text+= '('+$($routes[i]).data('street')+') ';
            text+= $($routes[i]).text()
            destinatonItemFunc(text, $($routes[i]).data('id'), $($routes[i]).data('type'), $($routes[i]).data('lat'), $($routes[i]).data('lng'), $($routes[i]).data('price'));
            $box = $("#destinations > div:last-child");
            //$box.find(".number_street").children('select').append('<option value=""></option>');
    ...
    if(latitudes.length>1&&longitudes.length>1) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if($('input[name="order_value"]').val().length)
                {
                    priceCalculate();
                }
                else
...
$('.switchery-search').on('change', function(){
    if($('.switchery-search').is(':checked')){
        $('#taxi_panel').show()
    }else{
        $('#taxi_panel').hide()
    }
...
function calculateAndDisplayRoute() {
    ...
    real_longitudeArray_length = 0;
    for (i = 0; i < longitudeArray.length; i++) {
        if(longitudeArray[i].value!="") {
            index = i;
            real_longitudeArray_length++;
        }
    }
    console.log(real_longitudeArray_length)

    if(real_longitudeArray_length == 1){
        console.log(latitudeArray[index].value+' '+longitudeArray[index].value);
        firstMarker = new google.maps.Marker({
            position: new google.maps.LatLng(latitudeArray[index].value, longitudeArray[index].value),
            //position: {lat: latitudeArray[index].value, lng: longitudeArray[index].value},
            map: map,
            title: 'A'
        });
        ...
        map.setCenter(new google.maps.LatLng(latitudeArray[index].value, longitudeArray[index].value));
    }else {
        ...
        var longitude_last = longitudeArray.length - 1;
        var latitude_last = latitudeArray.length - 1;
        var origin_long = longitudeArray[0].value;
        var origin_lati = latitudeArray[0].value;
        for (i = longitude_last; i > -1; i--) {
            if (longitudeArray[i].value) {
                var destination_long = longitudeArray[longitude_last].value;
                var destination_lati = latitudeArray[latitude_last].value;
                break;
            }
        }
        if (destination_long == null) {
            var destination_long = origin_long
            var destination_lati = origin_lati
        }
...
function addTaxiMarkers(marker_data){
    ...
    if(marker_data.banned_taxi_status == '1'){
        imageType = 'taxi_pin_red.png';
    }
    else {
        if (marker_data.order_status == null) {
            ...
            switch (marker_data.order_status) {
                case '2':
                    imageType = 'taxi_pin_blue.png';//accepeted
                    break;
                case '3':
                    imageType = 'taxi_pin_purple.png';//reached
                    break;
                case '8':
...
$.getScript("https://maps.googleapis.com/maps/api/js?key=AIzaSyAVXfCCk56gvY1zUf_vqHXn1i1mE2ebLu4&callback=initialize")
    .done(function( script, textStatus ) {
        if(requestMethod == 'get'){
            console.log('get');

            $('select[name="order_type"]').trigger('change');
            $('.number_street').trigger('change');
        }
        $('#customer_phone').trigger('keyup');
        $('#isCurrentTime').trigger('change');
        $('.switchery-search').trigger('change');
