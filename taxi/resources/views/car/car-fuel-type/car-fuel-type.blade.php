            ...
            <div class="content-wrapper">
                <div class="row">
                    <div class="col-md-6">
                        <div class="panel panel-white">
                            <div class="panel-heading">
                                <h5 class="panel-title">Yanacaq növü növü yarat</h5>
                            </div>
                            ...
                            <div class="panel-body">
                                ...
                                @if ($errors->any())
                                    ...
                                    @endforeach
                                @endif
                                <form action="{{ route('postCarFuelTypeEdit',['id' => 0,'code' => $module->code]) }}"
                    ...
                    <div class="col-md-6">
                        <div class="panel panel-white">
                            <div class="panel-heading">
                                <h5 class="panel-title">Yanacaq Növləri</h5>
                            </div>
                            <form action="{{ route('getModuleSearch',['code'=> $module->code,'viewMain' => 'car','view'=>'car-fuel-type.car-fuel-type' ]) }}"
                                        ...
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                <input type="text" name="name" value="{{request()->input('name')}}"
                                                       class="form-control" placeholder="Benzin">
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                            <div class="form-group">
                                    ...
                                    <div>
                                        <table class="table table-bordered table-striped table-hover">
                                            ...
                                            <tr>
                                                ...
                                                <th>
                                                    <a href="{{ Url('module/search/'.$module->code.'/car/car-fuel-type.car-fuel-type ') }}?_token={{ csrf_token() }}&name={{ request()->get('name')}}&selected={{ request()->get('selected')}}&status={{ request()->get('status') }}&column_name=name&order_type={{ request()->get('order_type') == 'asc' ? 'desc' : 'asc' }}&perPage={{ request()->get('perPage') }}">
                                                        <i class="@if( request()->get('column_name') == 'name') sort-amount-up icon-sort-amount-desc @endif"></i>
                                                        Ad</a></th>
                                                <th>
                                                    <a href="{{ Url('module/search/'.$module->code.'/car/car-fuel-type.car-fuel-type ') }}?_token={{ csrf_token() }}&name={{ request()->get('name')}}&selected={{ request()->get('selected')}}&status={{ request()->get('status') }}&column_name=sort&order_type={{ request()->get('order_type') == 'asc' ? 'desc' : 'asc' }}&perPage={{ request()->get('perPage') }}">
                                                        <i class="@if( request()->get('column_name') == 'sort') sort-amount-up icon-sort-amount-desc @endif"></i>Sıralama</a>
                                                </th>
                                                <th>
                                                    <a href="{{ Url('module/search/'.$module->code.'/car/car-fuel-type.car-fuel-type ') }}?_token={{ csrf_token() }}&name={{ request()->get('name')}}&selected={{ request()->get('selected')}}&status={{ request()->get('status') }}&column_name=status&order_type={{ request()->get('order_type') == 'asc' ? 'desc' : 'asc' }}&perPage={{ request()->get('perPage') }}">
                                                        <i class="@if( request()->get('column_name') == 'status') sort-amount-up icon-sort-amount-desc @endif"></i>
                                                        Status</a></th>
                                                <th>
                                                    <a href="{{ Url('module/search/'.$module->code.'/car/car-fuel-type.car-fuel-type ') }}?_token={{ csrf_token() }}&name={{ request()->get('name')}}&selected={{ request()->get('selected')}}&status={{ request()->get('status') }}&column_name=selected&order_type={{ request()->get('order_type') == 'asc' ? 'desc' : 'asc' }}&perPage={{ request()->get('perPage') }}">
                                                    ...
                                                    <td>
                                                        <div class="btn-group">
                                                            ...
                                                            <a class="btn btn-default"
                                                               href="{{ route('getCarFuelTypeEdit',['id' => $result->id,'code' => $module->code]) }}"><i
