                ...
                <div class="col-md-12">
                    <div class="panel panel-white">
                        <div class="panel-heading">
                            <h5 class="panel-title">Rayonu redaktə et</h5>
                        </div>
                        ...
                        <div class="panel-body">
                            @if(Session::has('success-message'))
                                ...
                                @if ($errors->any())
                                    ...
                                    @endforeach
                                @endif
                            <form action="{{ route('postRegionDistrictEdit',['id' => $result->id,'code' => $module->code]) }}" class="form-horizontal" method="post" accept-charset="utf-8">
                                @csrf
                                    ...
                                    <div class="col-lg-10">
                                        <select name="city" class="select-search form-control">
                                            @foreach($cities as $city)
                                                <option @if($city->id == $result->city) selected @endif  value="{{$city->id}}">{{$city->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
