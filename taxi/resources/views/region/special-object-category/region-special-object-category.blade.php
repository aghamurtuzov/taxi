            ...
            <div class="content-wrapper">
                <div class="row">
                    <div class="col-md-6">
                        <div class="panel panel-white">
                            <div class="panel-heading">
                                <h5 class="panel-title">Kateqoriya yarat</h5>
                            </div>
                            <div class="panel-body">
                                ...
                                @if ($errors->any())
                                    ...
                                    @endforeach
                                @endif
                                <form action="{{ route('postRegionSpecialObjectCategoryEdit',['id' => 0,'code' => $module->code]) }}" class="form-horizontal" method="post" accept-charset="utf-8">
                                    @csrf
                    ...
                    <div class="col-md-6">
                        <div class="panel panel-white">
                            <div class="panel-heading">
                                <h5 class="panel-title">Xüsusi Obyektlərin Kateqoriyaları</h5>
                            </div>
                            <form action="{{ route('getModuleSearch',['code'=> $module->code,'viewMain' => 'region','view'=>'special-object-category.region-special-object-category' ]) }}" name="listForm" method="get" accept-charset="utf-8">
                                @csrf
                                <div class="panel-body">
                                    <div class="row">
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                <input type="text" name="name" value="{{request()->input('name')}}" class="form-control" placeholder="X5">
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                            <div class="form-group">
                                    ...
                                    <div>
                                        <table class="table table-bordered table-striped table-hover">
                                            <thead>
                                            <tr>
                                                <th><a href="{{ Url('module/search/'.$module->code.'/region/special-object-category.region-special-object-category') }}?_token={{ csrf_token() }}&name={{ request()->get('name') }}&status={{ request()->get('status') }}&column_name=name&order_type={{ request()->get('order_type') == 'asc' ? 'desc' : 'asc' }}&perPage={{ request()->get('perPage') }}">
                                                <i class="@if( request()->get('column_name') == 'name') sort-amount-up icon-sort-amount-desc @endif"></i> Ad</a></th>
                                                <th><a href="{{ Url('module/search/'.$module->code.'/region/special-object-category.region-special-object-category') }}?_token={{ csrf_token() }}&name={{ request()->get('name') }}&status={{ request()->get('status') }}&column_name=sort&order_type={{ request()->get('order_type') == 'asc' ? 'desc' : 'asc' }}&perPage={{ request()->get('perPage') }}">
                                                <i class="@if( request()->get('column_name') == 'sort') sort-amount-up icon-sort-amount-desc @endif"></i> Sıralama</a></th>
                                                <th><a href="{{ Url('module/search/'.$module->code.'/region/special-object-category.region-special-object-category') }}?_token={{ csrf_token() }}&name={{ request()->get('name') }}&status={{ request()->get('status') }}&column_name=status&order_type={{ request()->get('order_type') == 'asc' ? 'desc' : 'asc' }}&perPage={{ request()->get('perPage') }}">
                                            ...
                                            <tr>
                                                <td>{{$result->name}}</td>
                                                <td>
                                                    <span class="badge badge-flat badge-primary text-primary-600" style="border-color: rgb(-30, 255, 0);">{{$result->sort}}</span>
                                                </td>
                                                <td>
                                                ...
                                                <td>
                                                    <div class="btn-group">
                                                         <a class="btn btn-default"
                                                               href="{{ route('getRegionSpecialObjectCategoryEdit',['id' => $result->id,'code' => $module->code]) }}"><i
