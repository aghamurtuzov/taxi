            ...
            <div class="content-wrapper">
                <div class="row">
                    <div class="col-md-6">
                        <div class="panel panel-white">
                            <div class="panel-heading">
                                <h5 class="panel-title">Obyekt Kateqoriyası Yarat</h5>
                            </div>
                            ...
                            <div class="panel-body">
                                ...
                                @endif
                                <form
                                    action="{{ route('postRegionObjectCategoryEdit',['id' => 0,'code' => $module->code]) }}"
                    ...
                    <div class="col-md-6">
                        <div class="panel panel-white">
                            <div class="panel-heading">
                                <h5 class="panel-title">Obyektin kateqoriyası</h5>
                            </div>
                            <form
                                action="{{ route('getModuleSearch',['code'=> $module->code,'viewMain' => 'region','view'=>'objectCategory.region-object-category' ]) }}"
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-striped table-hover"
                                            ...
                                            <tr>
                                                <th><strong>№</strong></th>
                                                <th>
                                                    <a href="{{ Url('module/search/'.$module->code.'/region/objectCategory.region-object-category') }}?_token={{ csrf_token() }}&page={{request()->input('page')?request()->input('page'):1}}&name={{ request()->get('name') }}&status={{ request()->get('status') }}&column_name=name&order_type={{ request()->get('order_type') == 'asc' ? 'desc' : 'asc' }}&perPage={{ request()->get('perPage') }}">
                                                        <i class="@if( request()->get('column_name') == 'name') sort-amount-up icon-sort-amount-desc @endif"></i>
                                                        Ad</a></th>
                                                <th>
                                                    <a href="{{ Url('module/search/'.$module->code.'/region/objectCategory.region-object-category') }}?_token={{ csrf_token() }}&page={{request()->input('page')?request()->input('page'):1}}&name={{ request()->get('name') }}&status={{ request()->get('status') }}&column_name=sort&order_type={{ request()->get('order_type') == 'asc' ? 'desc' : 'asc' }}&perPage={{ request()->get('perPage') }}">
                                                        <i class="@if( request()->get('column_name') == 'sort') sort-amount-up icon-sort-amount-desc @endif"></i>
                                                        Sort</a></th>
                                                <th>
                                                    <a href="{{ Url('module/search/'.$module->code.'/region/objectCategory.region-object-category') }}?_token={{ csrf_token() }}&page={{request()->input('page')?request()->input('page'):1}}&name={{ request()->get('name') }}&status={{ request()->get('status') }}&column_name=status&order_type={{ request()->get('order_type') == 'asc' ? 'desc' : 'asc' }}&perPage={{ request()->get('perPage') }}">
                                                    ...
                                                    <td>
                                                        <div class="btn-group">
                                                            <a class="btn btn-default"
                                                               href="{{ route('getRegionObjectCategoryEdit',['id' => $result->id,'code' => $module->code]) }}"><i
